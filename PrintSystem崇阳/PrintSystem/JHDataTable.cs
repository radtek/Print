...
namespace PrintSystem
{
    ...
    public class JHDataTable
    {
        ...
        public static DataTable JHDataTableBydwbmdep(string dwbm, string depcode)
        {
            DataTable dt = null;
            string sql =
                string.Format("select *  from View_BasAsset4 where DWBM ='{0}' and SYGLBM='{1}' ORDER BY RZRQ DESC ",
                    dwbm, depcode);
        ...
        public static DataTable JHDataTableBystrdwbm(string listassid, string tt, string dwbm)
        {
            ...
            string sql = string.Format(
                "select *  from View_BasAsset4 where  DWBM='{2}' and KPBH in ({0}) and MC='{1}'", listassid, tt, dwbm);
            dt = DBUnity.AdapterToTab(sql);
            return dt;
        }
        ...
        public static DataTable GetDepDt(string dwbm)
        {
            string sql =
                string.Format(
                    "select VCDEPTCODE as DepID,VCDEPTNAME as DepName from BASDEPTCODE where UNITCODE='{0}' and VCDEPTNAME!='系统维护部'",
                    dwbm);
