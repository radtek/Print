...
namespace PrintSystem
{
    public partial class Main : MainForm
    {
       ...
       // private int offset_Y = 139 - 3;
        ...
        private int Pagex=300;
        private int Pagey=200;
...
//            str = "#71110520100300012#台式电脑主机#微型数字电子计算机#1#2010-03-01##在用#征收局#商璐#征收局#征收局#计算中心#########23#6880##";
             ...
             try
            {
                ...
                var padding = new Point(180 + PrinterConfig.XOFFSET, PrinterConfig.YOFFSET + 40);
                var p0 = new Point(PrinterConfig.XOFFSET + 15, PrinterConfig.YOFFSET);
                var p1 = new Point(PrinterConfig.XOFFSET, (int)PrinterConfig.GAP * 1 + PrinterConfig.YOFFSET+10);
                var p2 = new Point(PrinterConfig.XOFFSET, (int)PrinterConfig.GAP * 2 + PrinterConfig.YOFFSET+10);
                var p3 = new Point(PrinterConfig.XOFFSET, (int)PrinterConfig.GAP * 3 + PrinterConfig.YOFFSET+10);
                var p4 = new Point(PrinterConfig.XOFFSET, (int)PrinterConfig.GAP * 4 + PrinterConfig.YOFFSET+10);
                var p5 = new Point(PrinterConfig.XOFFSET, (int)PrinterConfig.GAP * 5 + PrinterConfig.YOFFSET+10);
                // Point p4=new Point(0,0);
                Brush b1 = Brushes.Black;
                var f1 = new Font("宋体", 8, FontStyle.Bold);
                var f0 = new Font("宋体", 15, FontStyle.Bold);
                using (Graphics graphics = e.Graphics)
                {   
                    graphics.DrawString("湖北生物科技职业学院", f0, b1, p0);
                    graphics.DrawString("资产编码： "+strslist[0], f1, b1, p1);
                    graphics.DrawString("资产名称： " + getlength(strslist[1]), f1, b1, p2);
                    graphics.DrawString("使用部门： "+strslist[6], f1, b1, p3);
                    graphics.DrawString("资产类型： "+strslist[2], f1, b1, p4);
                    graphics.DrawString("使用人： " + strslist[7], f1, b1, p5);
                    graphics.DrawImage(image, padding);
                    
                    //设置旋转中心点 graphics.TranslateTransform(panelRuleY.Width / 2, panelRuleY.Height / 2);
                    //设置旋转角度 graphics.RotateTransform(90);
                    //平移 graphics.TranslateTransform(i - 19 - panelRuleY.Height / 2 + 22 - (20-panelWork.Top), -3);
                    //画文字 graphics.DrawString(((int)(i - 19)).ToString(), font, new SolidBrush(Color.Black), 0, 0);
                    //恢复为默认场景 graphics.ResetTransform(); 
                }
            }
            catch (Exception exe)
            {
        ...
        private void btnprint_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var ps = new PaperSize("Custom Size 1", Pagex, Pagey);
        ...
        private List<string> GetRowsListAll()
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                if (dt.Rows[i]["Selected"].ToString() == "1")
                {
                    ...
                    str += dt.Rows[i]["DepName"] + "&";
                    str += dt.Rows[i]["SYR"] + "&";
                    string CFDDID = dt.Rows[i]["CFDD"].ToString();
                    string CFDD = "";
                    CFDD = GetCFDDNAME(CFDDID);
                    if (!string.IsNullOrEmpty(CFDDID))
                    {
                        if (CFDDID.Substring(0, 1) == "0" || CFDDID.Substring(0, 2) == "00")
                        {
                            if (CFDDID == "000")
                            {
                                CFDDID = "0";
